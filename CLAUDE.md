...
2. Load ONLY the skills NEXUS selects — never blind-load all 38
...
| **Creative Quality** | "[HOOK]", "[BODY]", "[RESOLUTION]", "[CTA]", "hook pattern", "HOOK_BLOCKLIST", "tone variant", "TONE_RULES", "virality score", "hookStrength", "completionProxy", "shareability", "seoScore", "ViralitySignal", "VIRALITY_SCORE_RUBRIC", "captionDraft", "CAPTION_RULES", "soundSuggestion", "storyboard scene", "comfyPrompt", "faceless_broll", "kinetic_text", "scroll-stop", "pattern interrupt" |
...
> This cluster entry is tracked separately to signal that `swarmxq-ci-release-architect`
> handles all CI/CD gate sequencing, GitHub Actions, and SwarmXQ-specific release protocol.
> It is counted in the Cluster 8 total of 4 if applicable, or as #39 when installed.
...
- **TONE_RULES completeness**: `TONE_RULES` in `video-orchestrator.ts` must contain ALL 8 variants: `contrarian`, `urgent`, `educational`, `cinematic`, `warm`, `minimal`, `faceless_broll`, `kinetic_text`. Missing variants must be added before any pipeline change ships.
...
- The operator taxonomy (`Relay`, `Pilot`, `Architect`, `Forge`, `Oracle`, `Auditor`, `Lab`) maps to the TypeScript `MODEL_OPERATOR_MAP` — keep them in sync; never use V5 operator names (`SENTINEL`, `CANVAS`, `LEDGER`, `PROPHET`, `EVOLVER`)
...
- **Evolver agents** (meta-evolution) must write a memory note to `.serena/memories/` after every evolution cycle; never evolve silently
...
| `OLLAMA_NUM_THREADS` | `3` (WSL2) | Leave 1 core for OS + WSL2 hypervisor; 3 threads = ~15% higher tok/s than 4 on WSL2 |
| `OLLAMA_KEEP_ALIVE` | `0` (global) / `5m` (Pilot only) | Global evict-after-run; Pilot override in startup-enhanced.sh |

> **Why `OLLAMA_NUM_THREADS=3` not 4?** WSL2 shares cores with the Windows host. Using all 4 threads
> causes hypervisor preemption stalls. On bare-metal Linux: use 4 (`grep -qi microsoft /proc/version` to detect).
...
> `NEXUS` → removed (Python agent orchestrator, not a model operator) · `LEDGER` → `Auditor` ·
> `PROPHET` → removed · `EVOLVER` → `Lab`
...
# Ollama CPU performance (WSL2, 4-core, CPU-only) — set all before starting Ollama
...
OLLAMA_NUM_THREADS=3               # 3 of 4 cores; reserves 1 for WSL2 hypervisor + OS
...
cat .serena/memories/MEMORY.md          # session index
cat .serena/memories/project_v6.2.21.md # or most recent project_v*.md

# 3. Environment check
awk '/MemAvailable/ {printf "MemAvailable: %d MB\n", $2/1024}' /proc/meminfo  # RAM headroom
...
pnpm -F swarmx-api vitest run             # 0+ (grows from Priority 4)
...
pnpm -F swarmx-dashboard next build       # 10+ routes, zero build errors
...
# → post Priority 3: ≤10 hits, all documented as intentional escape hatches
...
| ~~7~~ | ~~**Smoke Renderer Certification Ceiling + Per-Stage Zod Validation**~~ | ✅ V6.2.49 — `renderer-certification.ts` clamps `ffmpeg_text_smoke` at `TECHNICALLY_VALID`; `certifyProductionPack()` and `certifyReadyToPost()` route through `clampCertificationTier()`; `stage-schemas.ts` validates planning/scripting/storyboard results; `job.stageValidationTrace` persists validation outcomes |
| ~~8~~ | ~~**S1: Voice Benchmark**~~ | ✅ V6.2.49 — `scripts/voice-benchmark.ts` writes ranked JSON report; `voice-benchmark-report.ts` reads + freshness-checks + ranks; `selectVoiceProvider()` consults the report when `SWARMX_TTS_PROVIDER=auto`; `/api/system/health` exposes `voice.benchmark` block; neural tier always preferred over synthetic_fallback |
...
- Ollama perf vars (`OLLAMA_NUM_PARALLEL`, `OLLAMA_FLASH_ATTENTION`, `OLLAMA_KV_CACHE_TYPE`, `OLLAMA_NUM_THREADS`) not set before starting Ollama service
...
- ~~`/api/system/health` not reading warmup status file → add `readWarmupStatus()` call~~ ✅ V6.2.26
- ~~Dashboard cold-start ETA hard-coded as 140 s regardless of warmup state → read from API~~ ✅ V6.2.26
- ~~Caption `soundSuggestion` not validated for URL/artist patterns at generation time~~ ✅ V6.2.26 (validated at generation in caption-generator + Series Pass C quality gate)

## Medium Impact — Log in memory note; address opportunistically

- ~~`stageViralityAndCaption()` result absent from job list API response (only in job detail)~~ ✅ V6.2.26 (verified: `queue.listJobs()` returns full VideoJob including `viralitySignal`)
- ~~Dashboard `VideoJobCard` missing `viralitySignal.overall` score badge~~ ✅ V6.2.26
- ~~`print()` or `logging.basicConfig()` in `src/swarmx/` Python layer → migrate to `structlog`~~ ✅ V6.2.36
- ~~Agent tool calls lacking idempotency — retry could produce duplicate side effects~~ ✅ V6.2.37 (stable memory IDs in learn_from_run + evolver)
- ~~WSL2 vs bare-metal thread count not auto-detected in `startup-enhanced.sh`~~ ✅ V6.2.31 (grep -qi microsoft /proc/version → OLLAMA_NUM_THREADS=3/4)
- ~~Ollama perf vars logged at boot but not included in `/api/system/health` response~~ ✅ V6.2.38 (env.ts schema + config.ollamaPerf in health endpoint)

---

# VERIFIED GROUND TRUTH (V6.2.21 — 2026-07-17)
...
| Env fail-fast (`src/lib/env.ts`) | Zod schema for ~15 top vars; `loadEnv()` wired in `server.ts`; formatted fatal + exit(1) on invalid config |
...
| Video orchestrator r7 (`video-orchestrator.ts`) | VOT-09 through VOT-13 correctness pass complete |
| Video queue SINGLE-VIDEO LOCK | `MAX_CONCURRENT_JOBS=1` enforced; idempotency by `clientRequestId`; 4h TTL cleanup |
| Dashboard: 12 named states | "Render Failed" included; `errorCodeHint` helper; `[HOOK]/[BODY]/[RESOLUTION]/[CTA]` renderer; caption char counter |
| Quality gates all green | api tsc · types tsc · dashboard tsc · vitest (52) · 5 regressions · dashboard build (10 routes) |
...
- **Quality gate results** — which passed, which were skipped and why
- **Host profile** — RAM at session start; `startup-enhanced.sh` active?; Ollama online?; Ollama CPU perf vars set?
