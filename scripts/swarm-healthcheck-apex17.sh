...
warn_check() {
  ...
  if "$@" 2>/dev/null; then
    ...
    echo -e "  ${YELLOW}⚠${NC} $label"; WARN=$((WARN + 1))
  fi
}
...
# ── 2. Canonical model registration ──────────────────────────────────────────
echo ""
...
check_model() {
  local tag="$1"
  echo "$MODELS" | grep -qE "^${tag}(:latest)?$"
}

warn_check "Relay (route-phi4-lite-q4km-prod) registered" check_model "route-phi4-lite-q4km-prod"
warn_check "Pilot (instruct-phi4-pro-q8-prod) registered" check_model "instruct-phi4-pro-q8-prod"
warn_check "Forge (code-qwen25-pro-q5km-prod) registered" check_model "code-qwen25-pro-q5km-prod"
warn_check "Oracle (reason-deepseekr1-pro-q5km-prod) registered" check_model "reason-deepseekr1-pro-q5km-prod"

# Check for stale -scar models that should have been migrated
STALE_FOUND=0
for legacy in phi4-router-lite-scar phi4-fast-scar qwen-worker-scar deepseek-reasoner-scar; do
  if echo "$MODELS" | grep -qE "^${legacy}(:latest)?$"; then
    echo -e "  ${YELLOW}⚠${NC} Legacy -scar model still present: $legacy"
    STALE_FOUND=$((STALE_FOUND + 1))
  fi
done
if [[ $STALE_FOUND -eq 0 ]]; then
  echo -e "  ${GREEN}✓${NC} No legacy -scar models found in Ollama"
  PASS=$((PASS + 1))
else
  echo -e "    ${YELLOW}Run: bash scripts/migrate-to-r7.sh --rename-only${NC}"
...
echo ""
echo -e "${CYAN}API${NC}"
warn_check "API /health responds" curl -sf "$API_URL/health" -o /dev/null --max-time 5
warn_check "API /api/system/health responds" curl -sf "$API_URL/api/system/health" -o /dev/null --max-time 5
...
if [[ -f "$REPO_ROOT/models/registry.yaml" ]]; then
  if grep -E '^[[:space:]]*ollama_tag:.*-scar' "$REPO_ROOT/models/registry.yaml" > /dev/null 2>&1; then
    echo -e "  ${YELLOW}⚠${NC} registry.yaml still has -scar in ollama_tag values"
    WARN=$((WARN + 1))
  else
