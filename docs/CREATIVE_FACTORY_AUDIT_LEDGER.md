...
| P1 | V4 requires canonical APEX tags in active setup paths | Makefile/install/verify/healthcheck/docs still referenced legacy triad defaults | Canonical operator map | Active setup paths now use APEX tags and `verify-release` runs a non-mutating model registry/Modelfile gate. |
| P1 | V4 requires all eight tones to be accepted by the workflow | API route schema accepted six tones while orchestrator rules covered eight | Orchestrator tone contract | `POST /api/video/jobs` and dashboard form now include `faceless_broll` and `kinetic_text`; regression checks assert the route source. |
| P1 | V4 requires safe artifact serving | File route defaulted unknown extensions to `video/mp4` | Explicit media allowlist | `/api/video/files/:filename` now serves only `.mp4` and `.webm`; unsupported extensions return `415 unsupported_media_type`. |
| P1 | V4 requires typed creative-intelligence records without duplicating durable systems | Workflow/run state was durable, but CreativeDNA, tournaments, variants, agent specs, and blackboard records were not first-class collections | Existing local snapshot/JSONL persistence | Added shared types, registry seeds, local-state collections, and read endpoints for those records. |
| P1 | V4 package manifests specify directive filenames | Existing package emitted older QC/provenance sidecar names | Directive names plus compatibility copies | FFmpeg production packages now emit `quality-report.json`, `rights-manifest.json`, `platform-manifest.json`, `voice-lineage.json`, `template-lineage.json`, and `thumbnail.jpg`. |
...
- `pnpm --filter @swarmx/api run test:models` passed when rerun outside sandbox.
