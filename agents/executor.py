"""
agents/executor — SwarmX V5.9 Async Parallel Executor
=======================================================
Executes a list of steps concurrently using asyncio.gather.

CHANGES V5.9 vs V5.8:
  [ENH-01] EventBus integration: publishes STEP_START and STEP_COMPLETE /
    STEP_FAILED events when SWARM_HOME is configured — provides full
    observability in the TUI without changing the execution contract.
  [ENH-02] execute_parallel() now accepts an optional `timeout_per_step`
    parameter (default None = no per-step timeout). When set, each step is
    wrapped in asyncio.wait_for so a single hung step cannot stall all others.
  [ENH-03] Result dicts include `duration_s` for downstream quality scoring
    and performance tracing.
  [PRESERVED] All V5.8 fixes and async correctness retained.
"""
from __future__ import annotations

import asyncio
import inspect
import os
import time
from typing import Any, Callable, Optional

_SWARM_HOME = os.environ.get("SWARM_HOME", "")


def _publish_event(kind: str, payload: dict) -> None:
    """Publish to event bus when SWARM_HOME is configured."""
    if not _SWARM_HOME:
        return
    try:
        from pathlib import Path
        from swarmx.event_bus import publish  # type: ignore[import]
        publish(Path(_SWARM_HOME), kind, payload)
    except Exception:
        pass


async def execute_parallel(
    steps: list[Any],
    route: Callable,
    *,
    return_exceptions: bool = True,
    timeout_per_step: Optional[float] = None,   # [ENH-02]
) -> list[dict[str, Any]]:
    """
    Execute all steps concurrently via asyncio.gather.

    Args:
        steps:             List of step payloads to dispatch.
        route:             Callable(step) → result. May be sync or async.
        return_exceptions: If True, exceptions are captured per-step.
        timeout_per_step:  [ENH-02] Per-step wall-clock timeout in seconds.
                           None = no timeout.

    Returns:
        List of dicts: [{"step": ..., "result": ..., "error": ..., "duration_s": ...}]
    """
    async def _call(step: Any) -> dict[str, Any]:
        t0 = time.monotonic()
        step_label = str(step)[:60] if not isinstance(step, dict) else str(step.get("action", step))[:60]
        _publish_event("step.start", {"step": step_label})  # [ENH-01]
        try:
            if inspect.iscoroutinefunction(route):
                coro = route(step)
            else:
                coro = asyncio.to_thread(route, step)

            if timeout_per_step is not None:
                result = await asyncio.wait_for(coro, timeout=timeout_per_step)
            else:
                result = await coro

            duration_s = round(time.monotonic() - t0, 3)
            _publish_event("step.complete", {"step": step_label, "duration_s": duration_s})  # [ENH-01]
            return {"step": step, "result": result, "error": None, "duration_s": duration_s}  # [ENH-03]

        except asyncio.TimeoutError:
            duration_s = round(time.monotonic() - t0, 3)
            err = f"TimeoutError: step exceeded {timeout_per_step}s"
            _publish_event("step.failed", {"step": step_label, "error": err, "duration_s": duration_s})
            return {"step": step, "result": None, "error": err, "duration_s": duration_s}

        except Exception as exc:
            duration_s = round(time.monotonic() - t0, 3)
            err = f"{type(exc).__name__}: {exc}"
            _publish_event("step.failed", {"step": step_label, "error": err, "duration_s": duration_s})
            return {"step": step, "result": None, "error": err, "duration_s": duration_s}

    return await asyncio.gather(*[_call(step) for step in steps])


def execute_parallel_sync(
    steps: list[Any],
    route: Callable,
    *,
    timeout_per_step: Optional[float] = None,
) -> list[dict[str, Any]]:
    """Synchronous wrapper for execute_parallel (for legacy / CLI callers)."""
    try:
        return asyncio.run(execute_parallel(steps, route, timeout_per_step=timeout_per_step))
    except RuntimeError:
        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            return pool.submit(
                asyncio.run,
                execute_parallel(steps, route, timeout_per_step=timeout_per_step),
            ).result()


__all__ = ["execute_parallel", "execute_parallel_sync"]
